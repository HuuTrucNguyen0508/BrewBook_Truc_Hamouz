-- RAG System Migration (Fixed Version)
...
-- Row Level Security for new tables (with IF NOT EXISTS handling)
alter table public.external_sources enable row level security;
do $$
begin
  if not exists (select 1 from pg_policies where tablename = 'external_sources' and policyname = 'external_sources read') then
    create policy "external_sources read" on public.external_sources for select using (true);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'external_sources' and policyname = 'external_sources insert') then
...
do $$
begin
  if not exists (select 1 from pg_policies where tablename = 'recipe_embeddings' and policyname = 'recipe_embeddings read') then
    create policy "recipe_embeddings read" on public.recipe_embeddings for select using (true);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'recipe_embeddings' and policyname = 'recipe_embeddings insert') then
...
do $$
begin
  if not exists (select 1 from pg_policies where tablename = 'recipe_generations' and policyname = 'recipe_generations read own') then
    create policy "recipe_generations read own" on public.recipe_generations for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'recipe_generations' and policyname = 'recipe_generations insert own') then
